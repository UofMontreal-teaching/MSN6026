...
p <- p + geom_histogram( aes(x=x),size=1.0, alpha = 0.5 , bins=50 , fill = "green")
...
#+ prior graph EN , echo = FALSE

p <- ggplot(data=mydata, aes(x=x))+ geom_line(aes(y=y),colour='red',size=1.5)

p <- p + theme_solarized()

p <- p +  xlab('Universe of hypotheses for IH mean height (cm)')+
  ylab('Density')+
  theme(axis.title.x=element_text(size=14))+
  theme(axis.title.y=element_text(size=14))+
  theme(axis.text.x=element_text(size=12 , hjust = 0.5))+
...
#+ likelihood graph EN, echo = FALSE
...
#+ posterior simulation EN, echo = FALSE , message = FALSE , warning = FALSE
...
#+ posterior theo EN , echo = FALSE , message = FALSE , warning = FALSE
...
#+ posterior simulation2 EN , echo = FALSE , message = FALSE , warning = FALSE
